...
The null hypothesis is formulated in terms of $b(\boldtheta)$ which denotes the expected background yield, and distribution. It depends on the ensemble of \emph{nuisance parameters} ($\boldtheta$), which represent the statistical and systematic uncertainties on the background predictions. %theoretical and empirical modeling of these sources. 
...
From this process, result of which is shown in figure \ref{fig:sbweight}, one can see that the number of events observed in the mass region in which the Higgs signal is expected to apear is below the background expectation.%an under--fluctuation of the backgrounds is clearly visible.
